...
genshin-OS（Chao-OS）是一个基于 Rust 语言从零开发的微内核操作系统模拟器。系统在单个 Linux 进程中完整模拟了操作系统的四层抽象：硬件层（虚拟 CPU、MMU、定时器、磁盘）、服务层（进程管理、内存管理、文件系统、设备驱动）、交换层（消息总线与内核路由）、用户交互层（Shell 与 TUI 监控器）。
...
\textbf{进程管理}。实现完整的 Fork-Exec-Exit-Wait 进程生命周期模型，摒弃简单粗暴的 Spawn 模式。每个用户程序（包括 ls、mkdir、cat 等内置命令）都必须经过 Fork 创建、Exec 加载、运行、Exit 退出、Wait 回收的完整流程。PCB（进程控制块）管理进程状态机（Creating→Ready→Running→Blocked→Zombie），支持多线程 TCB。SMP 双核调度器支持 RoundRobin、FIFO、Priority、SJF、MLFQ 五种策略，per-CPU 量子管理和跨核去重。
...
\textbf{文件系统}。实现四层文件系统栈——FileService（消息驱动的顶层服务）、VFS（基于 HashMap 的 inode 树，支持文件和目录的创建/查找/删除，通过 Serde JSON 持久化到 \texttt{.genshin-vfs.json}）、File（Vec<u8> 内存缓冲 + VirtualDisk 扇区映射）、VirtualDisk（基于 \texttt{.genshin-disk.img} 文件的 512B 块设备）。FileDescriptorManager 维护每进程的 fd 表，支持 open/close/read/write 操作。启动时自动从 \texttt{programs/} 目录导入 .asm 源文件到 VFS。
...
\begin{itemize}
    ...
    \item \textbf{外部依赖}：crossbeam-channel（多生产者多消费者无界通道，用于消息总线）、serde 与 serde\_json（VFS 树的 JSON 序列化/反序列化）。均为 MIT/Apache-2.0 双重许可的工业级库。
    \item \textbf{内存占用}：运行时约 50--100 MB（含 4MB 物理内存模拟、两个 4096 页表、VFS 树、磁盘缓存）。
\end{itemize}

\subsubsection{开发环境}

\begin{itemize}
    \item \textbf{编程语言}：Rust（edition 2021）。选择 Rust 的核心原因：(1)~所有权系统和借用检查器在编译期消除数据竞态和悬垂指针，适合并发密集的操作系统开发；(2)~零成本抽象的 enum/match/Arc<Mutex<T>> 模式完美匹配消息驱动的架构；(3)~Cargo 的依赖管理和测试框架无需 Makefile/CMake 等额外构建工具。
    ...
    \item \textbf{版本控制}：Git。采用 conventional commits 规范（\texttt{fix:} / \texttt{feat:} / \texttt{docs:} / \texttt{refactor:} / \texttt{chore:}），每个逻辑修改独立提交。仓库共 211 次提交，提交信息包含问题分析和修复说明。
...
本项目严格遵循微内核（Microkernel）设计哲学：内核（Kernel）仅做一件事——消息路由。系统调用、进程调度、内存分配、文件操作、设备驱动均以独立的用户态服务形式运行在各自的线程中。这种设计的核心优势是隔离性：一个服务的崩溃（如 FileService 在解析路径时 panic）不会影响其他服务（ProcessService 仍可调度进程）。L4 家族（seL4、Pistachio）和 QNX 等工业级微内核均采用此架构。

内核路由器的实现极为精简——仅 51 行代码（\texttt{src/services/kernel.rs}）。它在消息总线上阻塞等待 Envelope，根据 KernelMsg 枚举的变体将消息转发到四个专用通道（process\_tx、intr\_tx、memory\_tx、file\_tx）。内核不解析消息内容、不维护任何状态、不执行业务逻辑——它是真正的「消息交换机」。

\textbf{2. 消息总线：统一的通信语言}

所有跨模块通信强制通过 MessageBus trait 定义的接口：\texttt{send()}（发后即忘）、\texttt{send\_request()}（请求-响应）、\texttt{subscribe()}（订阅）。LockedBus 实现基于 \texttt{Arc<Mutex<Vec<Sender<Envelope>>>>}——订阅者列表由 Mutex 保护，消息通过 crossbeam-channel 的 unbounded 通道传递。

KernelMsg 枚举（7 个变体）是整个系统的通信语言：Syscall（用户系统调用）、Interrupt（硬件中断，含 Timer、PageFault、SyscallTrap）、Process（进程服务请求，含 30+ 子变体覆盖调度、IPC、同步、生命周期）、Memory（内存分配、映射、换页）、File（文件 CRUD、目录操作）、Device（设备读写）。任何模块只能使用这个枚举中的类型发送消息，不能调用其他模块的函数。这确保了接口的强制统一性——审查一个模块的行为只需检查它如何处理 KernelMsg。

Envelope 信封机制支持两种通信模式：fire-and-forget 用于通知（如 Timer 发送时钟中断），with-response 用于查询（如 Shell 的 \texttt{send\_and\_wait} 发送 ps 请求并等待进程列表）。响应通过 Envelope 内嵌的 \texttt{response\_channel}（一个 crossbeam \texttt{Sender<Response>}）传递，调用方用 \texttt{recv\_timeout(3s)} 阻塞等待。

\textbf{3. 两层译码：硬件与内核的职责分离}

系统调用的处理采用两层译码模型。第一层：VirtualCPU 执行 INT 0x80 时，仅保存寄存器到 \texttt{syscall\_regs}、设置 \texttt{syscall\_pending=true}、停机——它不知道 R0=18 代表 listdir，也不关心。第二层：定时器中断的 step loop 检查 \texttt{syscall\_pending} 后，调 \texttt{handle\_file\_syscall(cpu, r0, r1, r2)}，由 \texttt{match r0} 分派到 24 个 \texttt{syscall\_xxx()} handler。这种分离的核心理由是硬件层不应知道操作系统策略。真实的 x86 CPU 上，INT 0x80 只触发中断切换到内核态，系统调用号的解释完全由内核完成。我们的模拟保持了同样的抽象边界。

\textbf{4. Fork-Exec 模型：为什么不用 Spawn}

进程创建采用 Unix 经典的 Fork-Exec-Wait 三步模型，而非 Windows 式的 Spawn（CreateProcess 一步到位）。具体流程：ForkProcess 克隆父进程的页表和 CPU 状态，子进程 R0=0 标识其为 child，从父进程 PC 继续执行；ExecProcess 替换子进程的页表，加载 .asm 程序，重置 CPU PC=0，加入就绪队列；WaitChild 让父进程阻塞直到子进程退出为 Zombie，然后回收资源。

选择 Fork-Exec 而非 Spawn 的设计理由有三。其一，正交性：Fork 只做「复制」，Exec 只做「替换」，职责单一，便于测试和调试。其二，灵活性：Fork 后 Exec 前可以执行任意操作（如重定向文件描述符），为未来的管道和重定向功能预留空间。其三，教学价值：这是 Unix 操作系统的核心设计，理解 Fork-Exec 是理解 Unix 进程模型的关键。一个关键的实现细节是：\texttt{fork\_impl} 创建子进程后不立即调度，等待 \texttt{exec\_impl} 加载代码后才加入就绪队列，避免无代码进程被调度导致的页错误→Zombie 竞态。

\textbf{5. 系统调用的两路分发}

系统调用处理分为两条路径。文件/设备类（R0=10--18、208--211）走消息总线：handler 从进程虚拟地址 0x100 读取路径参数，发送 KernelMsg::File 或 KernelMsg::Device 到消息总线，Kernel 路由到 FileService 或 DeviceService，响应返回后 handler 将结果写入进程虚拟内存 0x200，设 R2 为字节数。进程/同步类（R0=0、100--102、200--205）不走总线，直接操作本地数据（\texttt{process\_table}、MMU、\texttt{SyncManager}）。区分依据是数据局部性：进程和同步的数据在 ProcessService 内部，走总线是浪费；文件数据在 FileService，必须走总线。

\textbf{6. 同步与异步的权衡}

当前 INT 0x80 系统调用都是同步的——CPU 停机等待 handler 返回后才继续执行下一条指令。这简化了 .asm 程序的编写（每个操作线性执行，不需要回调或事件循环），代价是 CPU 在等待 I/O（如磁盘读写）时完全阻塞。真正的微内核（如 seL4）使用异步 IPC 加 Notification 通知机制。鉴于 .asm 程序仅 3--15 条指令且数据量极小（文件内容 <4KB），同步等待的开销（\texttt{recv\_timeout(10ms)}）可以忽略。未来若支持网络栈或大文件，异步模型将更有价值。

\textbf{7. VFS 持久化策略}

VFS 树结构使用 Serde JSON 持久化到 \texttt{.genshin-vfs.json}。选择 JSON 的理由有二：可调试性——开发者可以直接用文本编辑器查看文件系统状态；简单性——derive 宏零代码实现序列化。代价是 JSON 格式比二进制大约 3--5 倍，但对于一个 inode 数不超过 200 的小规模文件系统，这点开销可忽略。磁盘镜像 \texttt{.genshin-disk.img} 使用二进制格式（512B 扇区），因为文件内容本身就是二进制数据，JSON 的 base64 编码反而浪费。这种「元数据 JSON + 数据二进制」的混合持久化策略平衡了可调试性和效率。
\subsubsection{关键技术与算法}

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{SMP 多核调度}。双 CPU 独立调度，采用 RR 时间片轮转算法（quantum = 3 ticks）。\texttt{dequeue\_next()} 从就绪队列取进程时跳过已在其他 CPU 运行的 PID；每 CPU 维护独立的 \texttt{last\_running} 状态，用于状态机转换。
    \item \textbf{TOCTOU 所有权转移}。信号量释放时不是简单地 count++，而是扫描进程表查找 \texttt{Blocked(WaitingForLock\{lock\_addr == sem\_id\})} 的等待者，将所有权直接转移给它。这种设计消除了多核环境下「释放后立即被调用者抢回」的竞态窗口。
    \item \textbf{CAS 无锁同步}。信号量和互斥锁的 wait/signal 操作全程使用 \texttt{AtomicU32/U64::compare\_exchange\_weak} 实现，不依赖 std::sync::Mutex 保护，在单线程模拟中保证了正确性，也为未来真正的多线程并发预留了正确的基础。
    \item \textbf{VFS inode 树}。基于 \texttt{HashMap<u64, VFSNode>} 的 inode 表，每个目录节点的 \texttt{children: HashMap<String, u64>} 实现子节点快速查找。\texttt{lookup\_path()} 从根开始逐级解析路径。通过 Serde JSON 持久化到 \texttt{.genshin-vfs.json}。
    \item \textbf{系统调用直接路径}。INT 0x80 触发的软中断不经过消息总线，而是由 \texttt{handle\_timer\_interrupt} 的 CPU 步进循环直接调用 \texttt{handle\_file\_syscall()}。这避免了每条 INT 指令都产生一个 SyscallTrap 消息通过总线→Kernel→ProcessService 的完整链路，消除了 SyscallTrap 消息泛滥导致的服务无响应问题。
\end{enumerate}
...
\begin{itemize}
    \item \texttt{KernelMsg}——所有消息的统一枚举，包含 Syscall、Interrupt、Process、Memory、File、Device 七个变体，是整个系统的通信语言。
    \item \texttt{PCB}（Process Control Block）——进程控制块，记录进程状态（Creating→Ready→Running→Blocked→Zombie）、页表映射、CPU 寄存器快照、父进程 PID、打开文件列表。
    \item \texttt{VFSNode}——虚拟文件系统节点，包含 inode 编号、节点类型（File/Directory/SymLink）、权限位、父目录 inode、子节点映射、数据块号列表。
    \item \texttt{Semaphore} 与 \texttt{MutexLock}——基于原子变量的同步原语。前者用 AtomicU32 维护计数器，后者用 AtomicU64 跟踪持有者 PID（u64::MAX 表示未锁定）。
    \item \texttt{Envelope}——消息信封，封装 KernelMsg 消息体、请求 ID、响应通道 Sender。支持 fire-and-forget 和 with-response 两种构造模式。
\end{itemize}
...
\begin{enumerate}[label=(\arabic*)]
    \item Timer 独立线程以 100Hz 频率（10ms 间隔）向消息总线发送 \texttt{Interrupt::Timer}。
    \item Kernel 路由器将中断转发到 ProcessService 的 \texttt{intr\_rx} 通道。
    \item ProcessService 主循环的 \texttt{try\_recv()} 收到中断，调用 \texttt{handle\_timer\_interrupt()}。
    \item 遍历两个虚拟 CPU（\texttt{for cpu\_id in 0..cpu\_count}）：每 CPU 先调 \texttt{schedule(cpu\_id)} 从就绪队列选进程，经过去重检查后标记为 Running，再执行最多 3 条指令（\texttt{cpu.step()}）。
    \item 执行过程中若 \texttt{cpu.syscall\_pending} 为真，则直接调用 \texttt{handle\_file\_syscall} 分发系统调用，无需经过消息总线。
    \item 检查 CPU 停机的进程：若非 Blocked 状态，则标记为 Zombie，释放持有的信号量/互斥锁。每 tick 扫描一个 Zombie 进程并回收其页帧和 PCB。
\end{enumerate}

对于文件系统操作，其流程略有不同：用户进程执行 \texttt{INT 0x80}（R0=18\,listdir）→ ProcessService 发送 \texttt{FileRequest::ListDir} 到消息总线 → Kernel 路由到 FileService → FileService 调用 \texttt{VFS.lookup\_path} 解析路径 → 收集子节点名称 → 通过 Envelope 的响应通道返回 \texttt{ResponseData::StringList} → ProcessService 将文件名列表（换行分隔）写入进程虚拟内存 0x200，设 CPU 的 R2 寄存器为字节数 → 进程通过后续 \texttt{INT 0x80}（R0=2\,print\_str）读取 0x200 并打印。
