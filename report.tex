...
\begin{itemize}
    \item 操作系统：Linux（Arch Linux / Ubuntu 22.04+）
    \item 运行时：Rust 编译的二进制文件，无外部运行时依赖
    \item 终端：支持 ANSI 转义序列的终端模拟器
\end{itemize}

\subsubsection{开发环境}

\begin{itemize}
    \item 编程语言：Rust（edition 2021）
    \item 编译器：rustc 1.70+
    \item 构建工具：Cargo
    \item 版本控制：Git
    \item 编辑器：VS Code / Neovim
\end{itemize}
...
本系统采用\heiti{微内核架构}，核心思想是\heiti{最小化内核态功能，最大化用户态服务}。内核（Kernel）仅负责消息路由，不执行任何业务逻辑。所有系统功能（进程管理、内存管理、文件系统、设备管理）均作为独立的用户态服务运行，彼此之间通过消息总线（MessageBus）进行异步通信。
