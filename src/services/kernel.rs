...
pub struct Kernel {
    ...
    intr_tx: Sender<Envelope>,
    memory_tx: Sender<Envelope>,
    file_tx: Sender<Envelope>,
}

impl Kernel {
    pub fn new(bus: Arc<dyn MessageBus>) -> (Self, Receiver<Envelope>, Receiver<Envelope>, Receiver<Envelope>, Receiver<Envelope>) {
        let receiver = bus.subscribe();
        let (ptx, prx) = unbounded();
        let (itx, irx) = unbounded();
        let (mtx, mrx) = unbounded();
        let (ftx, frx) = unbounded();
        (Self { bus, receiver, process_tx: ptx, intr_tx: itx, memory_tx: mtx, file_tx: ftx }, prx, irx, mrx, frx)
    }
    ...
    fn route(&self, envelope: Envelope) {
        ...
        match msg {
            KernelMsg::Interrupt(_) => { let _ = self.intr_tx.send(envelope); }
            KernelMsg::Process(_) | KernelMsg::Syscall(_) => { let _ = self.process_tx.send(envelope); }
            KernelMsg::Memory(_) => { let _ = self.memory_tx.send(envelope); }
            KernelMsg::File(_) => { let _ = self.file_tx.send(envelope); }
            KernelMsg::Device(_) => {}
        }
    }
}
